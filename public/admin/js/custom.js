$(document).ready(function () {
    ...
    $(document).on("click", ".updateBannerStatus", function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                // alert(resp);
                if (resp['status'] == 0) {
                    $('#banner-' + banner_id).html("<i style='font-size: 25px;' class='mdi mdi-bookmark-remove' status='Inactive'></i>")
                }
                else if (resp['status'] == 1) {
    ...
    $(document).on("click", ".updateCouponStatus", function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                // alert(resp);
                if (resp['status'] == 0) {
                    $('#coupon-' + coupon_id).html("<i style='font-size: 25px;' class='mdi mdi-bookmark-remove' status='Inactive'></i>")
                }
                else if (resp['status'] == 1) {
    ...
    $(document).on("click", ".updateFilterStatus", function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                // alert(resp);
                if (resp['status'] == 0) {
                    $('#filter-' + filter_id).html("<i style='font-size: 25px;' class='mdi mdi-bookmark-remove' status='Inactive'></i>")
                }
                else if (resp['status'] == 1) {
    ...
    $(document).on("click", ".updateAttributeStatus", function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                // alert(resp);
                if (resp['status'] == 0) {
                    $('#attribute-' + attribute_id).html("<i style='font-size: 25px;' class='mdi mdi-bookmark-remove' status='Inactive'></i>")
                }
                else if (resp['status'] == 1) {
    ...
    // Show Courier Name and Tracking if the Order Status is set to "Shipped" by the admin
    $("#courier_name").hide();
